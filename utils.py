...
def All_patch(Data1, Data2, patchsize, pad_width, Label):
    [m1, n1, l1] = np.shape(Data1)
    Data2 = Data2.reshape([m1, n1, -1])
    [m2, n2, l2] = np.shape(Data2)
    for i in range(l1):
...
def output_metric(tar, pre):
    ...
    return OA*100, AA_mean*100, Kappa*100, AA*100
...
def train_epoch(model,train_loader, criterion, optimizer):
    ...
    tar = np.array([])
    pre = np.array([])
    for batch_idx, (batch_data11, batch_data21, batch_target) in enumerate(train_loader):
...
def valid_epoch(model, valid_loader, criterion,k=1):
    objs = AverageMeter()
    top1 = AverageMeter()
    tar = np.array([])
    pre = np.array([])
    ft=0
    count=0
    for batch_idx, (batch_data11, batch_data21,  batch_target) in enumerate(valid_loader):
        batch_data11 = batch_data11.cuda()
        batch_data21 = batch_data21.cuda()
        batch_target = batch_target.cuda()
        #_,_,batch_pred,_,_ = model(batch_data11, batch_data21)
        batch_pred = model(batch_data11, batch_data21)
        ...
        if(k==2):
            if count == 0:
                ft =  batch_pred.detach().cpu().numpy()
                count = 1
            else:
                ft = np.concatenate((ft,  batch_pred.detach().cpu().numpy()))
    return tar, pre,ft
